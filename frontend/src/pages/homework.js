    ...
    <div className="homework">
      <div style={{ paddingTop: 10, display: "flex" }}>
        <Button
          size="large"
          startIcon={<ArrowBack />}
          onClick={() => props.history.push("/homeworks")}
        >
          Zpět
        </Button>
        <DoneButton
      ...
      <div>
        ...
        <p>
          Termín:{" "}
          {format(new Date(homework.dueTime), "EEEE d. MMMM y H:mm", {
            locale: cs,
          })}
        </p>
        ...
        {homework.user === user.username ? (
          <EditHomework
            postID={id}
            prevData={homework}
            onSubmit={getHomework}
          />
        ) : (
          [<h3>Tvůrce:</h3>, <p>{homework.userFullName}</p>]
        )}
        <hr />
        <AddComment disabled={false} id={id} />
        <hr />
        <Comments
          comments={homework.comments.map((item) => {
