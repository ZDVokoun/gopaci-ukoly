...
export async function handler(event) {
    ...
    try {
        ...
        const subscriptions = (await users.find({ subscriptions: { $exists: true }, groups: {$in: [req.group]}, notifyAbout: { $in: ["homeworks"] }, username: { $not: {$regex: payload.username} } }, {subscriptions: 1}).toArray()).map(item => item.subscriptions).flat(2)
